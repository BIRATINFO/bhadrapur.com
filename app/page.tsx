...
export default async function Page() {
  ...
  const roadblockRes = await fetch(`${backend_uri}/api/roadblocks/network/bhadrapur.com`, options)
  ...
  const [heroData, categoryBlocks] = await Promise.all([
    fetch(`${backend_uri}/api/posts/featured-news/bhadrapur.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => []),

    fetch(`${backend_uri}/api/posts/homepage/bhadrapur.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => [])
  ])
